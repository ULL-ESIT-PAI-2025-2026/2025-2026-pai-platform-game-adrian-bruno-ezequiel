...
export class CanvasDisplay {
     ...
     */
    updateViewport(center, worldWith, worldHeight) {
    ...
    drawBackground(grid) {
        ...
        for (let y = yStart; y < yEnd; y++) {
            for (let x = xStart; x < xEnd; x++) {
                ...
                this.context.drawImage(this.otherSprites, tileX, 0, this.scale, this.scale, screenPosition.x, screenPosition.y, this.scale, this.scale);
            }
        }
    }
    /**
    ...
    worldToCanvas(position) {
        return new Vector((position.x - this.viewport.left) * this.scale, (position.y - this.viewport.top) * this.scale);
    }
    /**
    ...
    drawPlayer(player) {
        ...
        this.context.drawImage(this.playerSprite, sprite * width, 0, width, height, x, y, width, height);
     ...
     */
    drawActors(actors, uiElements, numberOfCoins, numberOfCollectedCoints) {
        ...
        actors.forEach((actor) => {
            ...
            if (actor.type === 'player')
                return;
            this.context.drawImage(sprites, tileX, 0, width, height, screenPosition.x, screenPosition.y, width, height);
        });
        uiElements.forEach((actor) => {
            const width = actor.size.x * this.scale;
            const height = actor.size.y * this.scale;
            ...
            if (actor.type === 'lives') {
                ...
                this.context.strokeText(`${numberOfCollectedCoints}/${numberOfCoins}`, x + this.context.canvas.clientWidth - this.scale * 4, y + this.scale);
                this.context.fillText(`${numberOfCollectedCoints}/${numberOfCoins}`, x + this.context.canvas.clientWidth - this.scale * 4, y + this.scale);
            }
        });
    }
    /**
    ...
    drawFrame(frameData) {
        ...
        this.updateViewport(frameData.center, frameData.worldWidth, frameData.worldHeight);
        this.clearDisplay(frameData.status);
        this.drawBackground(frameData.grid);
        this.drawActors(frameData.actors, frameData.uiElements, frameData.numberOfCoins, frameData.numberOfCollectedCoins);
